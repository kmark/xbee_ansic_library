...
int parseParameter (const char *paramstr, int16_t request)
{
    ...
    if (paramstr[0] == '"')
    {
        ...
        {
            Serial.println( "Missing closing quote (\") in parameter.");
            return -EINVAL;
        }
    }
        ...
        && tolower( (unsigned char)paramstr[1]) == 'x')
    {
        ...
        while (*paramstr && length < sizeof(param))
        {
            ...
            if (j < 0)
            {
                Serial.println( "error parsing hex string");
                return -EINVAL;
            }
            *q++ = (uint8_t) j;
            paramstr += 2;
...
int xbee_cmd_callback( const xbee_cmd_response_t FAR *response)
{
    ...
    Serial.print("AT");
    Serial.print(response->command.str);
    Serial.print(" ");
            ...
            const addr64 FAR *ieee)
{
   ...
   const char *param;
    ...
    if (request < 0)
    {
      ...
      //newprintf( "Error creating request: %d (%" PRIsFAR ") \n",
...
void printATCmds( xbee_dev_t *xbee)
{
   ...
   switch (xbee->hardware_version & XBEE_HARDWARE_MASK)
   {
    ...
    case XBEE_HARDWARE_S3B:
        switch ((unsigned) (xbee->firmware_version & XBEE_PROTOCOL_MASK))
        {
            case XBEE_PROTOCOL_DIGIMESH:
                ...
                default:
                    newprintf( "Only DigiMesh supported on this hardware.\n");
                    break;
            }
            break;
        ...
        case XBEE_HARDWARE_S2B_PRO:
            switch ((unsigned) (xbee->firmware_version & XBEE_PROTOCOL_MASK))
            {
                ...
                default:
                    newprintf( "Unknown protocol for firmware version 0x%04x.\n",
            ...
            switch ((unsigned) (xbee->firmware_version & XBEE_NODETYPE_MASK))
            {
                ...
                default:
                    newprintf( "Unknown node type for firmware version 0x%04x.\n",
                        xbee->firmware_version);
                    break;
            }
            break;
    ...
    if (! flags)
    {
        newprintf( "Can't display menu for this hardware/firmware combination.\n");
    }
    else
    {
        newprintf( "Command\tDescription\n===================\n");
    ...
    newprintf( "\n\n");
    newprintf( "Valid command formats (AT prefix is optional, CC is command):\n\n");
    newprintf( " [AT]CC 0xXXXXXX (where XXXXXX is an even number of " \
        "hexadecimal characters)\n");
    newprintf( " [AT]CC YYYY (where YYYY is an integer, up to 32 bits)\n");
    newprintf( " [AT]NI \"Node ID String\" (where quotes contain string data)\n\n");
}
